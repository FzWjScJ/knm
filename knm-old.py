...
def start() -> int:
    if '-h' in sys.argv or '--help' in sys.argv or len(sys.argv) < 2:
        ...
        cmds:
            oldpca <pcafile> (<outfile>)           <old usb.capdata>转换pca的USB流量为data(文件)
            newpca <pcafile> (<outfile>)           <new usbhid.data>转换pca的data(文件)为USB流量
    ...
    if '-v' == sys.argv[1]:
        ...
        return 0
    elif 'oldpca' == sys.argv[1] or 'op' == sys.argv[1]:
        return analyse_oldpca(cmd=cmd)
...
# 提取usb流量信息
def analyse_oldpca(cmd) -> int:
    if 'oldpca' == sys.argv[1]:
        p = sys.argv.index('oldpca')
    elif 'p' == sys.argv[1]:
        p = sys.argv.index('op')
    if len(sys.argv) <= p+1:
        return 1 # cmd wrong
